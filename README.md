...
$R(s, t) = \frac{E\left[\left(X_s-\mu_s\right)\left(X_t-\mu_t\right)\right]}{\sigma_s\sigma_t}$
...
$R_f(\tau) = f(\tau) \times f^\star(-\tau) = \int_{-\infty}^{\infty}f(t+\tau)f^\star(t)\,dt = \int_{-\infty}^{\infty}f(t)f^\star(t - \tau)\,dt$


其中“\*”是卷积算符，$(\cdot)^\star$为取共轭。
